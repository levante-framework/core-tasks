...
onMounted(() => {
  ...
  const log = getBool('log', true);
  if (log) {
    setInterval(() => {
      // eslint-disable-next-line no-console
      console.log(`[stress] completed=${completed.value}/${totalCount.value} errors=${errors.value.length} active≈${Math.min(concurrency.value, totalCount.value - completed.value)}`);
    }, 2000);
  }

  // Autostart
  if (getBool('autostart', true)) {
    void run();
  }
});
</script>
