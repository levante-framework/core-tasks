describe('Matrix Reasoning Test with Fullscreen Mock and Screenshots', () => {
  ...
  function takeScreenshot(name) {
    if (Cypress.env('takeScreenshots')) {
      ...
      cy.screenshot(`${paddedCounter}_matrix_reasoning_${name}`, { 
        capture: 'viewport',
        overwrite: true 
      });
    }
  ...
  it('Matrix Reasoning test with fullscreen mocking and comprehensive screenshots', () => {
    const matrixReasoningUrl = 'http://localhost:8080/?task=matrix-reasoning';
    
    cy.visit(matrixReasoningUrl);
