  ...
  it('captures memory game with proper progression timing', () => {
    ...
    cy.get('body').then(($body) => {
      const continueButtons = $body.find(
        'button:contains("Continue"), button:contains("Next"), button:contains("Start"), button:contains("Begin")',
      );
    ...
    cy.get('body').then(($body) => {
      ...
      const gameElements = $body.find(
        'canvas, svg, [class*="card"], [class*="memory"], [class*="game"], button:visible',
      );
