describe('egma-math Ultra-Fast 150+ Test', () => {
  it('should capture 150+ screenshots with ultra-fast timing', () => {
    ...
    // Take 180 screenshots with ultra-fast timing
    for (let i = 1; i <= 180; i++) {
      cy.wait(500); // Reduced to 0.5 seconds
      
      // Ultra-fast interaction strategy
      cy.get('body').then(($body) => {
        ...
        // Strategy 4: Input fields
          ...
          cy.get('input[type="number"], input[type="text"]').first().clear().type('5');
          // Quick submit
          cy.get('button').contains(/Submit|Continue|Next/i).first().click().then(() => {}, () => {});
        }
      ...
      // Minimal wait after interaction
      cy.wait(100); // Just 0.1 seconds
      
      // Take screenshot
      cy.screenshot(`${i.toString().padStart(3, '0')}-step-${i}`);
      
      // Progress logging every 30 screenshots
      if (i % 30 === 0) {
    ...
    cy.log('Ultra-fast test completed! Captured 180+ screenshots');
  });
}); 