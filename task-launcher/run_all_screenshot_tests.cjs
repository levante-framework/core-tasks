...
console.log('🚀 Running all screenshot tests with fullscreen mocking...\n');
...
// Function to merge backup screenshots back into main directory
function mergeBackupScreenshots() {
  ...
  for (const entry of backupEntries) {
    ...
    const destPath = path.join(screenshotsDir, entry.name);
    
    if (entry.isDirectory()) {
      copyDir(backupPath, destPath);
    } else {
...
console.log('\n💡 Each test has its own directory with cleaned screenshots showing the full test execution flow.'); 