import { initTimeline, initTrialSaving, createPreloadTrials, checkFallbackCriteria } from '../shared/helpers';
// setup
...
export default function buildMemoryTimeline(config: Record<string, any>) {
  ...
  const downexPracticeTrial = (
    ...
    return {
      timeline: [
        getCorsiBlocks({ reverse, mode: 'display', isPractice: true, customSeqLength: seqlength, displayPrompt, preventAutoFinish: preventAutoFinishOnDisplay }),
        getCorsiBlocks({ reverse, mode: 'input', isPractice: true, animation, preventAutoFinish: true }),
  ...
  const downexInstructionsTimeline = {
    timeline: [
      ...
      downexPracticeTrial(false, 1, true, true, 'cursor'),
      downexPracticeTrial(false, 1, true, true),
      downexInstructions[1],
      downexPracticeTrial(false, 2, true, false, 'cursor'),
      downexPracticeTrial(false, 2, true),
      downexPracticeTrial(false, 2, true),
  ...
  const downexCorsiBlocksPracticeReverse = {
    timeline: [
      downexPracticeTrial(true, 2, true, false, 'cursor'),
      downexPracticeTrial(true, 2, true),
      downexPracticeTrial(true, 2, true),
    ]
  }
