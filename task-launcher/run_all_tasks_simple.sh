...
# Comprehensive Task Screenshot Capture - Simplified Version
...
generate_cypress_test() {
    local task_name=$1
    local test_file="cypress/e2e/${task_name}_simple_capture.cy.js"
...
run_ocr_cleanup() {
    local task_name=$1
    local screenshot_dir="cypress/screenshots/${task_name}_simple_capture.cy.js"
...
create_backup() {
    ...
    local backup_dir="all_tasks_simple_$timestamp"
...
# Main execution
main() {
    log "Starting comprehensive task screenshot capture (simplified version)"
    ...
    for task in "${TASKS[@]}"; do
        ...
        if timeout 300 npx cypress run --spec "cypress/e2e/${task}_simple_capture.cy.js" --browser chrome --headless; then
