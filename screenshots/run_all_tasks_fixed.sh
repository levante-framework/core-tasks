...
# Fixed version with proper associative array handling

# set -e  # Disable exit on error to continue with other tasks
...
BACKUP_DIR="all_tasks_fixed_$(date +%Y%m%d_%H%M%S)"

# Initialize associative array properly
declare -A TASKS
TASKS["egma-math"]="afc_with_slider"
TASKS["matrix-reasoning"]="afc_multi_choice"
TASKS["mental-rotation"]="afc_multi_choice"
TASKS["hearts-and-flowers"]="spatial_response"
TASKS["memory-game"]="corsi_blocks"
TASKS["same-different-selection"]="binary_choice"
TASKS["trog"]="afc_multi_choice"
TASKS["vocab"]="afc_multi_choice"
TASKS["theory-of-mind"]="afc_multi_choice"
TASKS["intro"]="instruction_only"
TASKS["roar-inference"]="afc_multi_choice"
TASKS["adult-reasoning"]="afc_multi_choice"
  ...
  it('should capture screenshots with smart interactions', () => {
    ...
    function performMathInteraction() {
      cy.get('body').then($body => {
        ...
        if ($body.find('input[type="range"]').length > 0) {
          // Random position on slider
          const randomValue = Math.floor(Math.random() * 100);
          cy.get('input[type="range"]').first().invoke('val', randomValue).trigger('change');
          cy.wait(1000);
        }
        
        // Then look for submit/continue buttons
    ...
    function performMemoryGameInteraction() {
      cy.get('body').then($body => {
        ...
        for (let selector of memorySelectors) {
          if ($body.find(selector).length > 0) {
            if (selector.includes('block')) {
              // Click a random block
              cy.get(selector).then($blocks => {
                if ($blocks.length > 0) {
    ...
    // Spatial response interaction (hearts-and-flowers)
    function performSpatialInteraction() {
      cy.get('body').then($body => {
        // Look for left/right response buttons
        const spatialSelectors = [
          'button[data-response="left"]',
          'button[data-response="right"]',
          '.response-left',
          '.response-right',
          '.jspsych-btn'
        ];
        ...
        for (let selector of spatialSelectors) {
          if ($body.find(selector).length > 0) {
            ...
            if (selector.includes('left') || selector.includes('right')) {
              const targetSelector = isLeft ? selector.replace('right', 'left') : selector.replace('left', 'right');
              if ($body.find(targetSelector).length > 0) {
                cy.get(targetSelector).first().click({ force: true });
                return;
              }
            }
            cy.get(selector).first().click({ force: true });
...
main() {
    ...
    for task_name in "${!TASKS[@]}"; do
        ...
        log "Processing task $task_count/${#TASKS[@]}: $task_name"
