describe('Vocabulary 240 Frames Capture', () => {
  ...
  const smartInteraction = () => {
    cy.get('body').then(($body) => {
      ...
      // Strategy 1: Vocabulary questions (highest priority)
      if (!interactionMade && $body.find('.image-medium').length >= 2) {
        cy.log('📚 Vocabulary question detected');
        if ($body.find('.correct').length > 0) {
          cy.log('✅ Clicking correct answer');
          cy.get('.correct').first().click({ force: true });
        } else {
          cy.log('🎲 Clicking first vocabulary option');
          cy.get('.image-medium').first().click({ force: true });
        }
  ...
  it('captures exactly 240 Vocabulary screenshots', () => {
    cy.visit('http://localhost:8080/?task=vocab', {
      timeout: 60000,
      onBeforeLoad(win) {
        // Mock fullscreen API
    ...
    const screenshotInterval = 2500; // 2.5 seconds (slightly faster to fit more frames)
    ...
    for (let i = 4; i < totalFrames; i++) { // Start from 4 since we already have 4 screenshots
      cy.wait(screenshotInterval).then(() => {
        // Take screenshot
        cy.get('body').then(($body) => {
          if ($body.find('.image-medium').length >= 2) {
            takeScreenshot(`frame-${i}-vocab-question`);
    ...
    // Final validation
    cy.then(() => {
      cy.log(`🎉 Vocabulary 240 frames capture completed! Total screenshots: ${screenshotCounter}`);
